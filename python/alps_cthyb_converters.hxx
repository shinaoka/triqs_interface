...
template <> struct py_converter<solve_parameters_t> {
 static PyObject *c2py(solve_parameters_t const & x) {
  ...
  PyDict_SetItemString( d, "thermalization_time", convert_to_python(x.thermalization_time));
 ...
 static solve_parameters_t py2c(PyObject *dic) {
  ...
  _get_optional(dic, "thermalization_time", res.thermalization_time   ,-1);
 ...
 static bool is_convertible(PyObject *dic, bool raise_exception) {
  ...
  std::vector<std::string> ks, all_keys = {"h_int","random_seed","max_time","thermalization_time","verbosity","imag_threshold","basis_rotation"};
  ...
  _check_optional <int           >(dic, fs, err, "thermalization_time", "int");
